<!--
Engineering deep-dive report on an internal ML system (SmartPhoto V4).
Shows: multi-source preamble (Confluence, GitHub PRs, Jira, Slack, source code) → two-level changes log (v2, v1) → ## 1. Conclusion (numbered H2) → ## 2. Key Findings with ### N.M. subsections → ## 3. Evidence.
Use to calibrate: how Conclusion density looks for an internal system investigation; how Confluence/PR/Slack sources are cited differently from external GitHub URLs.
Read to: one line before ## 2. Key Findings.
  grep -n "^## [0-9]" <this-file> | sed -n '2p'
-->
