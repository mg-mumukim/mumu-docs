<!--
Operational shutdown guide for a cloud service.
Shows: sources/changes preamble → Prerequisites section (numbered subsections P1–P4 with interface, URL, commands) → ordered checklist sections (Section N) each with step-by-step commands and verification steps.
Use to calibrate: guide/procedural document structure as opposed to analytical documents; how prerequisites are laid out; how command blocks and contact info appear in action items.
Read to: one line before the ordered checklist (fourth H2 heading).
  grep -n "^## " <this-file> | sed -n '4p'
-->
