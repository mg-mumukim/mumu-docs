# Photo Review AURA Squad — Handoff v5

**Session date:** 2026-04-30
**Coverage:** 04-16 ~ 04-30 (2026)

**Changes from v4**
- People: Nathan Lamb added (TL, SOUP); Xincen Hao corrected to Director of Engineering (stakeholder, not dev); Evan Keys corrected to Canvas team SWE Backend (not SOUP); Luma Marques added (QA Tester)
- WorkItems: `blockedBy` annotations added to Eval 6 run, Mock server, e2e testing; DLP→Eval 6 pool impact noted
- Dates: standardized to MM-DD format
...
| Notion | HPCNT workspace — search "AURA photo review"; MGAI PM-TL Sync 04-30 |
...
| Spec Kick-off | Engineering alignment meeting; Part 1 04-27, Part 2 04-29 |
...
| Link Lee | MLE | internal (transferred to Tailor ~04-21) | link.lee@match.com |
...
|---|---|---|---|
| Option 2 Decision Brief | 04-29 | Met | Brandon |
| Mock server + ML server design doc | 05-06 | Planned | Brandon |
| ML Deployment & integration testing | 05-11 (week of) | Planned | Brandon |
| SOUP Q2 capacity available | late May | Planned | SOUP |
| BE Release QA | 06-01 | Planned | — |

---

### WorkItems (current status as of 04-30)

| ID | Title | Owner | Status | blockedBy |
|---|---|---|---|---|
| WI-01 | rSRR Predictor report delivered to Trideep (notified 04-27, delivered 04-29) | Biggie | Done | — |
| WI-02 | Prompt v4.6.1 — Eval 4 feedback reflected | Biggie + Parker | Done | — |
| WI-03 | Eval 6 KB expansion — injection logic review | Biggie | In Progress | — |
| WI-04 | Prompt v5.0.0 restructure for Eval 6 | Biggie | In Progress | WI-03 (KB logic decision first) |
| WI-05 | Eval 5 | Juniper | Skipped | — |
| WI-06 | Eval 6 run | Biggie, Juniper, Parker | To Do | WI-03, WI-04, WI-31 |
...
| WI-13 | Workshop attendance (Palo Alto, 04-27 ~ 04-29) | Juniper | Done | — |
...
| WI-22 | Mock server + ML server design doc | Brandon | To Do (05-06) | contract alignment this week |
| WI-23 | ML Deployment & integration testing | Brandon | To Do (05-11 wk) | WI-22; full e2e blocked by SOUP Q2 |
...
| WI-31 | DLP compliance — AURA demo data (dry-run complete, 470 users retained) | Parker | In Progress (05-06) | — |
| WI-32 | Prompt Factory ↔ AURA ActionPlan schema contract | Parker | To Do | — |
| WI-33 | Lifestyle Concepts KB v2.0 (PR #260) | Claude Yoon | Done | — |
...
|---|---|---|---|
| Custom ML model descoped from MVP | Biggie, Mumu | 04-22 | Resolved |
| Serving direction = Option 2 (On-demand Sync) | Juniper, Jaini, Mumu | 04-24 | Resolved |
| Bottom sheet timing = next app open | Evan Keys, Jaini | 04-24 ~ 04-25 | Resolved |
| Coaching trigger = photo add/delete/replace only | Brandon, Evan Keys | 04-28 | Resolved |
| Cadence: next coaching on photo change, keep when unchanged | Juniper, Jaini, Brandon | 04-28 | Resolved |
| Repetitive cluster model: Best-keep + One-cluster-at-a-time | Jaini, Juniper, Trideep | 04-28 | Resolved |
| MVP language scope = English-only | Evan Keys, Juniper | 04-28 | Resolved |
| AURA LLM skip signal (status = success/skip) | Brandon | 04-28 | Resolved |
| Eval 5 skipped; next iteration = Eval 6 | Juniper | ~04-29 | Resolved |
| Gemini Pro kept as MVP fallback option | Biggie (+ 04-29 discussion) | 04-29 | Resolved |
| Bottom sheet dismiss policy: per-insight vs. full opt-out | Jaini | 04-28 | **Unresolved** — engineering preference: full opt-out |
...
| Markdown rendering effort (client side) | Trideep, Evan Keys | 04-28 | **Unresolved** — follow-up pending |
| OpenAI via Bedrock as LLM API alternative | Ben Shin (raised) | 04-29 | **Unresolved** — cost estimate in progress (Kevin) |

---

## Unresolved Items (carry into Stage 3 of next session)

1. **Bottom sheet dismiss policy**: Jaini deferred 04-28; engineering preference full opt-out; PM ratification pending.
...
4. **SOUP effort estimate**: Ryan Burns + Henry Au working session 04-30; MG AI schedule not yet planned.
5. **Eval 6 KB injection logic**: Injecting full 25-concept KB adds 13+ atoms/profile → excessive LLM load; KB usage logic + prompt v5.0.0 restructure in progress. Both block Eval 6 run.
