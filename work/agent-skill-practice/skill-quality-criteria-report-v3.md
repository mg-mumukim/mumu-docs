...
- Removed §6 Validation: skill testing is a separate work stream from reviewing whether a skill artifact is well-formed; §1–5 are all assessable from the document alone without running the skill.
- Updated Conclusion from six to five MECE groups.
- "Skill existence criteria" evidence repointed from §6 to §2 (Scope Validity), where it belongs.
...
A good skill is one that Claude selects at the right moment, delivers only information Claude does not already have, and reliably produces the intended behavior. The criteria that distinguish good from bad skills fall into five MECE groups: (1) Identity & Discoverability, (2) Scope Validity, (3) Information Architecture, (4) Instruction Quality, and (5) Compliance Reliability. Groups 1–4 apply to all skills. Group 5 applies only to discipline-enforcing skills.
...
A skill that encodes something Claude already does reliably consumes context tokens without adding value. The baseline test for novelty: did Claude fail at this task without the skill?
