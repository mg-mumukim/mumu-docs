...
| Max length | Under 64 characters | Over 64 characters (rejected by runtime) |
...
| Length (soft) | Ideally under 500 characters | Verbose without necessity (500 chars is a recommendation, not a hard limit) |
...
| Criterion | Good | Bad |
|---|---|---|
| Entry point | SKILL.md under 500 lines | Single file exceeding 500 lines |
...
| Large reference files | Table of contents at top of files over 100 lines | No navigation; Claude reads head-100 preview and misses content |
| Supporting files | Created only for heavy reference (100+ lines) or reusable executable scripts | Created for content that fits inline |
...
| Metric | Target | Limit |
|---|---|---|
| SKILL.md body length | Under 500 lines (performance degrades above this) | Split into separate files when approaching limit |
| Frequently loaded skill word count | Under 150 words | — |
| Standard skill word count | Under 500 words | — |
...
| Coverage | 3+ scenarios; triggering AND non-triggering cases | Happy path only |
| Volume | 10–20 test prompts, multiple trials each | Single trial |
