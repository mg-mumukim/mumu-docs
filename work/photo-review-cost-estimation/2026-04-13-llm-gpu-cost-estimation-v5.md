...
>
> **Changes from v4**:
> - Rewrote TL;DR: added pipeline diagram, replaced abbreviations (CSP/HA) with descriptive names (Savings Plan/always-on)
> - Removed Mode A/B/C labeling throughout; GPU sections now named by serving mode (Real-Time, Batch)
> - Collapsed "Three GPU Billing Modes" from Section 2 into Section 5 GPU Serving Scenarios
> - Rounded combined cost table totals ($33.1K → $33K, $70.1K → $70K, etc.)
> - Removed Assumption 7.11 (GPU vs API Quality Difference)
> - Expanded Assumptions 7.6 and 7.9 with throughput derivation detail (79 users/sec source, L40S extrapolation method)
...
In real-time mode, both stages fire when the user opens the app, so the GPU cluster must be always-on ($4.1K/month). In batch mode, eligible users are collected and processed once daily, so GPUs spin up for ~1 hour and shut down ($38-247/month).

| Scope | Serving Mode | API Cost | GPU Cost | Total | Within $50K? |
|-------|-------------|--------:|---------:|------:|:------------:|
| Test regions (USA+CAN+AUS) | Batch (GPU on-demand) | $33K | $50 | $33K | Yes |
| Test regions | Batch (GPU Savings Plan) | $33K | $38 | $33K | Yes |
| Test regions | Real-time (GPU always-on) | $66K | $4K | $70K | No |
| Global | Batch (GPU on-demand) | $163K | $247 | $163K | No |
| Global | Batch (GPU Savings Plan) | $163K | $188 | $163K | No |
| Global | Real-time (GPU always-on) | $325K | $4K | $329K | No |
...
| Compute Savings Plan (CSP, 24% off) | $1.414 | $1,018 |
| 3yr Reserved Instance (RI, hypothetical) | $0.804 | $579 |

Tinder uses Compute Savings Plan (CSP), not Reserved Instances (RI). The aura-demo cost dashboard defaults to 3yr RI, which is not a pricing option Tinder has purchased.
...
[!NOTE] this should be savings plan.
### Real-Time: Always-on HA Cluster

Real-time serving requires the GPU cluster to be available for immediate inference when a user opens the app. Production minimum: 4 pods (1 pod per availability zone x 4 AZs) running 24/7.
...
Monthly cost = 4 pods x $1.414/hr x 24h x 30d = **$4,073 (CSP)**

### Batch: On-Demand Burst

GPUs spin up, process the daily batch within 1 hour, and spin down. No always-on cluster needed. The number of GPUs is determined by how many are needed to finish within the 1-hour window: ceil(daily_calls / 284,400).
...
[!NOTE] is savings plan "burst"?
### Batch: Savings Plan Burst

Same burst pattern as on-demand but with CSP discount (24% off).
...
On day 1, all active users need to be processed at once (no one has been scored yet). For batch mode, this means spinning up more GPUs for a one-time burst. For real-time mode, the always-on 4-pod cluster absorbs the load (takes ~16h with 4 GPUs for global).

| Scope | Users | GPUs for 1h | GPU-Hours | On-Demand | CSP |
|-------|------:|------------:|----------:|----------:|----:|
...
| Scope | Serving Mode | API Cost | GPU Cost | Total | Within $50K? |
|-------|-------------|--------:|---------:|------:|:------------:|
| Test regions | Batch + GPU on-demand | $33K | $50 | $33K | Yes |
| Test regions | Batch + GPU Savings Plan | $33K | $38 | $33K | Yes |
| Test regions | Real-time + GPU always-on | $66K | $4K | $70K | No |
| Global | Batch + GPU on-demand | $163K | $247 | $163K | No |
| Global | Batch + GPU Savings Plan | $163K | $188 | $163K | No |
...
| Test regions | Real-time + GPU always-on | $31.4K | $0 (absorbed) | $31.4K |
| Global | Batch + GPU on-demand | $77.4K | $118 | $77.5K |
| Global | Real-time + GPU always-on | $154.8K | $0 (absorbed) | $154.8K |
...
Tinder uses Compute Savings Plan (CSP) for GPU instances. CSP discount for g6e is ~24% off on-demand. The aura-demo cost dashboard defaults to 3yr Reserved Instance ($0.804/hr, 57% off), but Tinder has not purchased GPU RI. Using RI pricing would understate GPU cost by 1.76x.
...
Real-time GPU serving requires 1 pod per availability zone x 4 AZs = 4 pods minimum, running 24/7. This does not apply to batch mode, where GPUs can be spun up on-demand.
