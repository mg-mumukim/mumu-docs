...
GPU cost depends on the serving mode: real-time requires an always-on 4-pod HA cluster ($3.9K/month), while batch only pays for GPU-hours needed to process the daily batch within 1 hour ($38-247/month).

| Scope | Serving Mode | API Cost | GPU Cost | Total | Within $50K? |
|-------|-------------|--------:|---------:|------:|:------------:|
| Test regions (USA+CAN+AUS) | Batch (GPU on-demand) | $33K | $50 | $33.1K | Yes |
| Test regions | Batch (GPU CSP) | $33K | $38 | $33K | Yes |
| Test regions | Real-time (GPU HA) | $66K | $3.9K | $69.9K | No |
| Global | Batch (GPU on-demand) | $163K | $247 | $163.2K | No |
| Global | Batch (GPU CSP) | $163K | $188 | $163.2K | No |
| Global | Real-time (GPU HA) | $325K | $3.9K | $328.9K | No |
...
- Monthly cost: 4 x $1.414/hr x 24h x 30 = $4,072 (CSP)
- Rounded: $3,870/month (using exact CSP rate)
...
| AURA (always-on estimate) | Single service | 4 GPU | g6e.xlarge | $3,870 |
...
In batch mode, GPU cost is less than 1% of the total — LLM API dominates. In real-time mode, GPU adds ~$3.9K fixed overhead.
