...
>
> **Changes from v5**:
> - Fixed GPU billing model per serving mode: real-time uses Savings Plan (24/7 committed), batch uses on-demand (burst). Removed invalid "Batch + Savings Plan" option.
> - Reduced GPU serving options from 3 to 2: Real-time (Savings Plan) and Batch (on-demand).
> - Updated TL;DR, Section 5, and Section 6 tables to reflect 2 options.
> - Updated Assumptions 7.7 and 7.8 to clarify which pricing applies to which mode.

---

## TL;DR

Each Photo Review request runs through two stages: a self-hosted GPU predicts concepts from the user's profile photos, then an LLM API generates human-readable coaching text from those concepts. Both costs are additive.

**Request -> [GPU: concept prediction] -> [LLM API: coaching text] -> Response**

In real-time mode, both stages fire when the user opens the app, so the GPU cluster must be always-on — Savings Plan pricing applies ($4K/month). In batch mode, eligible users are collected and processed once daily, so GPUs spin up for ~1 hour and shut down — on-demand pricing applies ($50-247/month).

| Scope | Serving Mode | API Cost | GPU Cost | Total | Within $50K? |
|-------|-------------|--------:|---------:|------:|:------------:|
| Test regions (USA+CAN+AUS) | Batch | $33K | $50 | $33K | Yes |
| Test regions | Real-time | $66K | $4K | $70K | No |
| Global | Batch | $163K | $247 | $163K | No |
| Global | Real-time | $325K | $4K | $329K | No |
...
## 2. GPU Self-Hosted Unit Cost

Baseline model: Qwen3-VL-8B SFT LoRA on NVIDIA L40S (g6e.xlarge, us-east-1).

### Per-Node Pricing

| Pricing Model | Rate ($/hr) | Monthly (1 pod, 24/7) | Applies to |
|---------------|------------:|----------------------:|------------|
| On-Demand | $1.861 | $1,340 | Batch (burst) |
| Compute Savings Plan (CSP, 24% off) | $1.414 | $1,018 | Real-time (always-on) |
| 3yr Reserved Instance (RI, hypothetical) | $0.804 | $579 | Not used |

Savings Plan (CSP) requires committed spend and is cost-effective for always-on workloads. On-demand is appropriate for batch burst where GPUs run for less than an hour per day. Tinder does not purchase Reserved Instances for GPU.

### Throughput

Each GPU processes 79 users/sec (full-forward) or ~94 users/sec (production-truncated, trunc=19). That is 284,400 users per GPU per hour at full-forward speed.
...
The GPU stage runs concept prediction before the LLM API call. How the GPU is deployed depends on the serving mode chosen in Section 4.

### Real-Time: Always-on Cluster (Savings Plan)

Real-time serving requires the GPU cluster to be available for immediate inference when a user opens the app. Production minimum: 4 pods (1 pod per availability zone x 4 AZs) running 24/7. Savings Plan pricing applies because the cluster runs continuously.

Each pod operates independently in its AZ, so total cluster throughput is 4x single-pod. The 4-pod cluster handles up to 27.3M users/day — well above global steady-state (1.26M/day).

Monthly cost = 4 pods x $1.414/hr x 24h x 30d = **$4,073**

### Batch: On-Demand Burst

GPUs spin up, process the daily batch within 1 hour, and spin down. No always-on cluster needed. On-demand pricing applies because usage is too short and infrequent to justify a Savings Plan commitment.

The number of GPUs is determined by how many are needed to finish within the 1-hour window: ceil(daily_calls / 284,400).

| Scope | Daily Calls | GPUs for 1h | GPU-Hours/Day | Monthly GPU-Hours | Monthly Cost (On-Demand) |
|-------|------------:|------------:|--------------:|------------------:|-------------------------:|
...
On day 1, all active users need to be processed at once (no one has been scored yet). For batch mode, this means spinning up more GPUs for a one-time on-demand burst. For real-time mode, the always-on 4-pod cluster absorbs the load (takes ~16h with 4 GPUs for global).

| Scope | Users | GPUs for 1h | GPU-Hours | On-Demand Cost |
|-------|------:|------------:|----------:|---------------:|
| Global | 18M | 64 | 63.3 | $118 |
| Test regions | 3.65M | 13 | 12.8 | $24 |

### CRS Production Comparison

For reference, two existing GPU-serving systems at Tinder that use the same always-on deployment pattern:
...
| Scope | Serving Mode | API Cost | GPU Cost | Total | Within $50K? |
|-------|-------------|--------:|---------:|------:|:------------:|
| Test regions | Batch | $33K | $50 | $33K | Yes |
| Test regions | Real-time | $66K | $4K | $70K | No |
| Global | Batch | $163K | $247 | $163K | No |
| Global | Real-time | $325K | $4K | $329K | No |

In batch mode, GPU cost is less than 1% of the total — LLM API dominates. In real-time mode, GPU adds ~$4K fixed overhead.

### Day-1 (Cold Start) — One-Time Total

| Scope | Serving Mode | API Cost | GPU Cost | Total |
|-------|-------------|--------:|---------:|------:|
| Test regions | Batch | $15.7K | $24 | $15.7K |
| Test regions | Real-time | $31.4K | $0 (absorbed) | $31.4K |
| Global | Batch | $77.4K | $118 | $77.5K |
| Global | Real-time | $154.8K | $0 (absorbed) | $154.8K |
...
For batch serving, the GPU stage must complete processing within 1 hour. The number of GPUs needed is calculated as ceil(daily_calls / 284,400) where 284,400 = 79 users/sec x 3,600 sec. The 79 users/sec figure is the full-forward throughput (conservative); production-truncated inference (~94 users/sec) would require ~16% fewer GPU-hours, but this document uses the conservative number.
- Value: 1-hour batch window, using 79 users/sec (full-forward)
- Source: Planning assumption (not a system constraint); throughput from Assumption 7.9

### 7.7 GPU Pricing by Serving Mode

Different GPU pricing models apply depending on the serving mode:
- **Real-time (always-on)**: Compute Savings Plan (CSP) at $1.414/hr per g6e.xlarge (24% off on-demand). Savings Plan requires committed spend but is cost-effective for 24/7 workloads.
- **Batch (burst)**: On-demand at $1.861/hr per g6e.xlarge. Burst usage of ~1 hour/day does not justify a Savings Plan commitment.

The aura-demo cost dashboard defaults to 3yr Reserved Instance ($0.804/hr, 57% off), but Tinder has not purchased GPU RI.
...
Real-time GPU serving requires 1 pod per availability zone x 4 AZs = 4 pods minimum, running 24/7. This does not apply to batch mode, where GPUs are spun up on-demand for the duration of the batch window only.
- Value: 4 pods, 24/7 (real-time); ceil(daily_calls / 284,400) pods for ~1h (batch)
...
Derived L40S figures used in this document (Qwen3-VL-8B, B=16):
...
GPU cost figures do not include EBS storage, data transfer, Karpenter startup latency, or CPU orchestration overhead. Those are for backend systems.

### 7.11 Eligible User Filtering (Not Applied)
