...
[!NOTE] Those are sequential component for different models. We use self-hosted GPU server for classification and uplift modeling task, then run LLM API to generate human-readable outcome. So, inform readers that we've added two numbers for total cost estimation.
Photo Review can serve via LLM API (Gemini 3.1 Flash-Lite) or self-hosted GPU (Qwen3-VL-8B on L40S). The two options are alternatives, not additive.

[!NOTE] Report total cost estimation with proper breakdown. i.e. $37K (API=$33K, GPU=$3.9K) Each option might need different combination of calculation.
| Scope | Serving Option | Monthly Cost | Within $50K Budget? |
|-------|---------------|-------------:|:-------------------:|
| Test regions (USA+CAN+AUS) | LLM API Batch | $33K | Yes |
| Test regions | LLM API Real-time | $66K | No |
| Test regions | GPU Self-hosted (4-pod HA) | $3.9K | Yes |
| Global | LLM API Batch | $163K | No |
| Global | LLM API Real-time | $325K | No |
| Global | GPU Self-hosted (4-pod HA) | $3.9K | Yes |

GPU self-hosted cost is infrastructure-based (4 pods x 24/7 regardless of traffic), not per-request. One 4-pod HA cluster handles both test-region and global steady-state traffic. Day-1 backfill adds a one-time LLM API spike of ~$77K (batch) or ~$155K (global, real-time) if using the API option; the GPU cluster absorbs backfill within existing capacity (3 GPUs needed for 24h window).
...
[!NOTE] Is this interpretation reasonable even if 4 pods are located at different AZs?
Throughput per GPU: 79 users/sec (full-forward) or ~94 users/sec (production-truncated, trunc=19). One GPU processes 284,400 users/hr (full-forward), so a 4-pod cluster handles up to 27.3M users/day — well above global steady-state (1.26M/day).
...
1. **First login after launch (cold start)** — The user has never received a Photo Review score.
...
Two serving options for LLM API. They differ in when the call happens and how much it costs.
...
[!NOTE] Self hosting is sequentially correlated with LLM API scenario. fix this.
Self-hosted inference using Qwen3-VL-8B on g6e.xlarge (L40S). Always-on 4-pod HA cluster.
...
### Caveats from GPU Investigation

[!NOTE] Caveats here are not for end-readers. Add these informations and others from source to Assumptions section.
The dashboard understates steady-state cost by ~36x ($107 vs $3,870/month) due to three issues:

1. RI pricing is hypothetical — Tinder uses CSP (24% discount), not 3yr RI (57% discount).
2. Billing model assumes spin-up/down — production GPU nodes run 24/7.
3. No HA minimum — production requires 4 pods (1/AZ), dashboard uses 1.

Additional issues: backfill ignores market coverage (always uses 18M DAU), GPU vs API comparison does not account for quality difference (88.2% accuracy vs frontier models), no infrastructure overhead (EBS, data transfer, Karpenter latency).
...
### 6.1 Maximum One Call per User per Day
...
### 6.6 GPU Pricing: CSP, Not RI

Tinder uses Compute Savings Plan for GPU instances. CSP discount for g6e is ~24% off on-demand.

- Value: $1.414/hr per g6e.xlarge (after CSP)
- Source: Jessica Hickey (Slack #cloud-infra-operations, 2025-12-30)

### 6.7 GPU HA Minimum: 4 Pods

Production GPU services require 1 pod per AZ x 4 AZs = 4 pods minimum, running 24/7.

- Value: 4 pods, 24/7
- Source: CRS PII Redaction and CRS Photo Suggestion deployment patterns; Jessica Hickey guidance

### 6.8 GPU Throughput: H100 to L40S Extrapolation
...
### 6.9 Eligible User Filtering (Not Applied)
