...
- HuggingFace model card: Qwen/Qwen3-VL-8B-Instruct (license: Apache 2.0)
- Slack #mgai-aura threads (model benchmarks, training logs, architecture discussions)

---

**Changes from v2**

- Added `[!NOTE]` markers above content that was new in v2, for reviewer visibility.

**Changes from v1**

- Added "Model Details" subsection to Step 3 (Custom Model Training): specifies the model used (Qwen3-VL-8B), its license, training method (LoRA), on-premise infrastructure, and data handling implications.
...
> [!NOTE]
> New in v2: "Model Details" subsection added below. Specifies model identity, license, training method, infrastructure, and data handling implications for legal review.

#### Model Details

The custom model is based on **Qwen3-VL-8B-Instruct**, an open-weight vision-language model developed by the Qwen team (Alibaba Cloud). The model is publicly available under the **Apache 2.0 license**, which permits commercial use, modification, and distribution without requiring derivative works to be open-sourced.

The model's Vision Transformer (ViT) backbone provides the ability to process both images and text jointly -- a requirement for the concept discovery and prediction objectives, which must analyze profile photos alongside textual profile elements. Downstream tasks such as causal inference consume the output of this model rather than processing raw user photos directly.

**Training method**: The model is adapted to the project's domain through LoRA (Low-Rank Adaptation) supervised fine-tuning. LoRA trains a small set of adapter parameters layered on top of the base model's frozen weights, rather than modifying the full model. The training data consists of the source data described in Step 1.

**Infrastructure and data handling**: Model training and inference are performed on HyperPod, Hyperconnect's internal GPU cluster (NVIDIA H100). Unlike the third-party LLM API vendors listed in Step 6 (Anthropic, OpenAI, Google), the custom model operates entirely within the organization's own infrastructure. No user data is transmitted to external vendors for model training or custom model inference. Data retention on HyperPod follows the same 3-month policy described in Step 8.

**Pre-trained model provenance**: The base model was pre-trained by the Qwen team on publicly available datasets independently of this project. The project's use of Tinder user data is limited to the LoRA fine-tuning phase; the base model's pre-training did not involve any Tinder or Match Group data.

**Prior deployment within the organization**: Variants of the same model family (Qwen VL series) have been previously deployed within the organization for Tinder's SmartPhoto v4 project, establishing an operational precedent for this model architecture.
