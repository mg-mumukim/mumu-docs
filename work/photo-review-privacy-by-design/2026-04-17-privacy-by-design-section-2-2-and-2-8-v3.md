...
- `note/source/2026-04-08-aura-research-agents.md`
- `note/source/2026-04-08-aura-demo-prompt.md`
...
- GitHub matchgroup-ai/aura-agent/reports/compliant (compliant Knowledge Base research reports; source for processing methodology and data compliance enforcement)
- Prior draft: Section 2.2 and 2.8 combined v2 (2026-04-17)

---

**Changes from v2**

- Updated source reference from `aura-agent/docs` to `aura-agent/reports/compliant` and revised all related references in Step 2.
- Added SQL-level data compliance enforcement details to Step 2 (step 3): geographic exclusion filters applied inline in SQL, GDPR anonymization filter (`is_anonymized = 0`), and minimum aggregate cell size (1,000 users per group) to prevent re-identification.
- Specified Gemini Vision API as the LLM for photo classification in research (Step 2, step 4) and added structured photo taxonomy detail.
- Added specific statistical methods to Step 2 (step 5): independent two-sample t-test, Cohen's d with 95% confidence intervals, Bonferroni correction for multiple comparisons, Simpson's Paradox checks across gender strata, and mandatory gender stratification.
- Replaced the generic pipeline summary note with a description of the standardized report format, the 6 report categories (BIO, PHOTO, BEHAVIOR, BEHAVIORAL, ENGAGEMENT, DEMOGRAPHIC), and the three-reviewer approval process (statistical rigor, methodology, data compliance).
- Updated the output description in Step 2 with precise aggregate metric types (mean rSRR per cohort, Cohen's d, 95% CIs, p-values) and the minimum cell size safeguard.
...
- **Research**: AI agents run aggregate statistical analyses to build a "Knowledge Base". All outputs are aggregate-level and contain no individually identifiable data. The Knowledge Base is a deployed artifact: Knowledge Base atoms are injected as context into the demo pipeline.
- **Model training**: Custom models discover domain concepts, predict their presence per user, and estimate causal treatment effects. These models are used in the research phase and are not deployed for the MVP scope.
- **Demo**: The LLM API (Gemini) directly analyzes user profile elements -- photos, bio, user-selected interests, badges, and demographic context -- and generates natural language coaching using Knowledge Base atoms as data-backed evidence. No custom model inference is performed online per user for the MVP. Photos are passed unblurred (required for expression/composition evaluation); no facial identification or biometric extraction is performed. Bio text may contain sensitive information, but coaching topics are pre-fixed to an approved non-sensitive concept list.
...
2. **Hypothesis formulation**: An engineer defines a research hypothesis -- e.g., "Do outdoor photos improve rSRR?" or "How fatal are sunglasses in profile photos?" User-selected interests (Step 1b) are among the profile elements included in hypotheses -- for example, to examine whether alignment between a stated interest (e.g., hiking) and a corresponding photo affects rSRR, or how backfire effects manifest when stated interests and profile content are inconsistent.
3. **Data querying**: The AI research agent translates the hypothesis into SQL queries and executes them against the Databricks warehouse, retrieving behavioral data and profile attributes for the target cohort. Data compliance is enforced inline within each SQL query: geographic exclusion filters (EU/EEA/CH/Réunion and US-Illinois excluded via NOT IN and JOIN filters), a GDPR anonymization filter (`is_anonymized = 0`) to exclude deleted-account records, and a minimum aggregate cell size of 1,000 users per group to prevent re-identification. All queries produce cohort-level output, not individual user rows.
4. **LLM-assisted classification** (when applicable): For hypotheses requiring visual or textual understanding (e.g., "Does a cluttered background reduce rSRR?"), the Gemini Vision API classifies profile photos of users in the cohort at scale using a structured photo taxonomy (e.g., full_body_formal, selfie_mirror, outdoor_solo). The classification results are joined with behavioral data for analysis.
5. **Statistical analysis**: The agent performs cohort comparison using independent two-sample t-tests, with Cohen's d effect sizes and 95% confidence intervals. Bonferroni correction is applied for multiple comparisons. All analyses include mandatory gender stratification and a Simpson's Paradox check to detect cases where aggregate-level findings reverse within gender strata.
6. **Report generation**: A single structured report is produced per hypothesis, covering Executive Summary, Data Compliance Statement, Methodology, Results, Limitations, Statistical Rigor Checklist (12 criteria), and Conclusions.
7. **Human review and curation**: Each report requires approval from three reviewers (statistical rigor, methodology, and data compliance) before its compliance_status is set to approved. Engineers additionally confirm the domain concept list to exclude concepts with legal concerns. Validated insights are curated into the Knowledge Base and are not exported or consumed by downstream systems (model training, demo) until review is complete.

Completed Knowledge Base reports are published under six categories -- BIO, PHOTO, BEHAVIOR, BEHAVIORAL, ENGAGEMENT, and DEMOGRAPHIC -- at https://github.com/matchgroup-ai/aura-agent/tree/main/reports/compliant.
...
- **Output**: "Knowledge Base" -- cumulative statistical insights about which profile elements (e.g., outdoor photos, clear face shots) contribute to positive dating outcomes (e.g., higher rSRR). All Knowledge Base outputs are aggregate-level statistics -- specifically, mean rSRR per cohort, Cohen's d effect sizes, 95% confidence intervals, and p-values per demographic segment. No individual user's data is identifiable in any report entry; the minimum cell size requirement (1,000 users per aggregate group) further prevents re-identification. The Knowledge Base is a deployed artifact: Knowledge Base atoms are injected as context into the LLM demo pipeline (Step 4) and cited in coaching recommendations as data-backed evidence.
- **LLM vendors**: See Step 6.

### Step 3: Custom Model Training

**MVP scope note**: Custom models described in this section are used in the research phase and are not deployed for the MVP scope. The MVP relies on the Knowledge Base (Step 2) and direct LLM API inference (Step 4) rather than custom model inference. Deployment scope for custom models will be determined after the MVP phase.
...
For individual users, the system generates personalized profile coaching. For the MVP, no custom model inference is performed online per user. The demo pipeline operates as follows:

1. **Knowledge Base context**: Research-backed Knowledge Base atoms (aggregate statistical insights from Step 2) are injected into the LLM prompt as evidence, providing data-grounded support for coaching recommendations.
2. **LLM API (Gemini)** directly analyzes the user's profile elements -- photos (visual analysis using built-in vision capability), bio, user-selected interests (Step 1b), badges, job, and school -- and generates personalized natural language coaching using the Knowledge Base atoms as cited evidence. User-selected interests are explicitly used to personalize photo recommendations: for example, if "hiking" is listed as an interest and no hiking photo is present, the LLM may recommend adding one; if "hiking" is listed but no hiking photo exists, coaching is tied to the stated interest rather than a generic recommendation.
...
|--------|----------|-------------|
| Knowledge Base | MGAI team (internal); demo pipeline | Aggregate-level statistical insights on profile elements and dating outcomes (cohort-level statistics; no individual user data). Deployed: Knowledge Base atoms are injected as context into the LLM demo pipeline and cited in coaching recommendations. |
| Custom Model | MGAI team (internal) | Trained models to predict dating concepts and estimate dating outcomes. Research artifact -- not deployed for the MVP scope. |
...
Project outputs that contain only aggregate or non-individual data -- Knowledge Base (cohort-level statistics with no individual user data), trained model weights, evaluation quality scores -- are retained for the project duration and are not subject to the above deletion cycle. The Knowledge Base is exempt because its outputs are aggregate-level statistics that do not constitute Personal Information; no individual user's data is identifiable in any Knowledge Base entry.
...
The concept prediction stage produces a derived dataset (user-profile/concept-score) in which each user record is associated with a predicted score for each domain concept. This dataset is stored on HyperPod and is used as input to the causal inference model. This artifact is generated during the research/training phase; the resulting trained models are not deployed for the MVP scope (see Step 3).
