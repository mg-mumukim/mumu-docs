...
- GitHub matchgroup-ai/aura-agent/docs (full research processing pipeline documentation; private repository -- not independently verified)
- Prior draft: Section 2.2 and 2.8 combined v1 (2026-04-17)

---

**Changes from v1**

- Added MVP scope note to Step 3 (Custom Model Training): custom models are used in the research phase and are not deployed for the MVP scope. Updated the Executive Summary "Model training" and "Demo" bullets and the Step 7 output table accordingly.
- Explicitly stated in Step 2 and Step 7 that all Knowledge Base outputs are aggregate-level statistics (no individual user data) and therefore do not constitute Personal Information.
- Clarified in Step 2, Step 4, and Step 7 that the Knowledge Base is a deployed artifact: Knowledge Base atoms are injected as context into the LLM demo pipeline and cited in coaching recommendations.
- Updated Step 4 to reflect the MVP demo flow: the LLM API directly analyzes user profile elements using Knowledge Base atoms as context, with no custom model inference performed online per user.
- Added user-selected profile interests (Step 1b) as an explicit input to Knowledge Base research hypotheses (Step 2) and to the demo personalization pipeline (Step 4).
- Added research processing pipeline detail to Step 2: SQL data retrieval, LLM-assisted classification, and statistical analysis with tool references, plus a pointer to full documentation.
...
3. **Data querying**: The AI research agent translates the hypothesis into SQL queries and executes them against the Databricks warehouse, retrieving aggregate data including profile elements (photos, bio, interests, behavioral metrics) and outcome signals (swipe history, rSRR) for the target cohort.
...
5. **Statistical analysis**: The agent performs correlation and confounder handling between the classified profile attributes and rSRR outcomes within each cohort, including p-values, effect sizes, sample size validation, and confounder analysis.
...
The research pipeline is orchestrated by the research-rigor skill, which coordinates SQL data retrieval, LLM-assisted image and text classification, and statistical comparison. Full pipeline documentation is available at https://github.com/matchgroup-ai/aura-agent/tree/main/docs.
