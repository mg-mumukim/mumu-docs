...
- GitHub catboost/catboost (license: Apache 2.0)
...
- Clarified that CatBoost is not a pre-trained model -- it is trained from scratch on project-specific data, unlike Qwen VL which is fine-tuned from a pre-trained base.

**Changes from v4**

- Added CatBoost (gradient boosting, Apache 2.0) as the model used for causal inference, clarifying the two-model architecture: Qwen VL for concept extraction, CatBoost for causal inference.
...
Two categories of open-source models are used in Step 3. Both are licensed under the **Apache 2.0 license**, which permits commercial use, modification, and distribution without requiring derivative works to be open-sourced.

**Concept extraction (objectives 1 and 2)** uses the **Qwen VL (Vision-Language) model family**, a series of open-weight models developed by the Qwen team (Alibaba Cloud). Multiple model variants within this family (differing in parameter size) are evaluated during development; the final production variant is selected based on accuracy and infrastructure constraints. The Qwen VL architecture provides vision-language capability -- the ability to process both images and text jointly -- which is required for concept discovery and prediction, as these tasks must analyze profile photos alongside textual profile elements.

**Causal inference (objective 3)** uses **CatBoost**, an open-source gradient boosting library developed by Yandex. Unlike the Qwen VL model, CatBoost is not a pre-trained model -- it is trained entirely from scratch on project-specific data. It operates on structured numerical data -- specifically, the concept scores produced by the Qwen VL model and the behavioral outcome data described in Step 1. It does not process raw photos or text directly.
