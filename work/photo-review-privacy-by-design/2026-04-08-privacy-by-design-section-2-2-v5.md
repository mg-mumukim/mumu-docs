...
**Causal inference (objective 3)** uses **CatBoost**, an open-source gradient boosting library developed by Yandex. CatBoost is a traditional machine learning model that operates on structured numerical data -- specifically, the concept scores produced by the Qwen VL model and the behavioral outcome data described in Step 1. It does not process raw photos or text directly.
